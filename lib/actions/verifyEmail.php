...
if (isset($q[0]['email'])){
    ...
    if ($mastertoken === $checktoken){
        ...
        $STH->update('users', array('is_verified'), array('is_verified' => 1, 'email' => $email), array('email', '=', ':email'));
        if ($STH->lastCount() > 0){
            echo "Thank you, $email has been verified!<br>";
        } else {
            echo "There was an error updating the database with your information.<br>";
        }
    } else {
        //Else, spit out error message
        echo "Sorry, but we couldn't verify $email_v, it didn't match the email in our records.<br>";
    }
} else {
